defmodule BankingApiWeb.UserControllerTest do
  ...
  describe "show - admin permission" do
    ...
    test "raises not found for not found ID", %{conn: conn} do
      ...
      assert_error_sent 404, fn -> get(conn, Routes.user_path(conn, :show, Ecto.UUID.generate())) end
    end
  end
  ...
  describe "show signed in user - admin permission" do
    ...
    test "renders own user", %{conn: conn, user: user} do
      ...
      conn = get(conn, Routes.user_path(conn, :show_signedin_user))
  ...
  describe "update user - admin permission" do
    ...
    test "ensures that ID can not be updated", %{conn: conn} do
      ...
      conn = put(conn, Routes.user_path(conn, :update, user), user: %{name: "Test new ID", id: Ecto.UUID.generate()})
      assert %{"id" => ^id} = json_response(conn, 200)["data"]
    ...
    test "raises error response when user not found", %{conn: conn} do
      ...
      assert_error_sent 404, fn -> put(conn, Routes.user_path(conn, :update, Ecto.UUID.generate()), user: @update_attrs)
      end
    end
  ...
  describe "update own user - default permission" do
    ...
    test "ensures that user with default permission can't change its permission", %{conn: conn, user: user} do
      %{:id => id} = user

      conn = put(conn, Routes.user_path(conn, :update_own_user), user: %{name: "Test new name", permission: "ADMIN"})
      assert %{"id" => ^id} = json_response(conn, 200)["data"]
    ...
    test "renders user when data is valid", %{conn: conn, user: user} do
      ...
      conn = put(conn, Routes.user_path(conn, :update_own_user), user: @update_attrs)
      assert %{"id" => ^id} = json_response(conn, 200)["data"]
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = put(conn, Routes.user_path(conn, :update_own_user), user: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete user with admin permission" do
    ...
    test "raises error response when user not found", %{conn: conn} do
      ...
      assert_error_sent 404, fn -> delete(conn, Routes.user_path(conn, :delete, Ecto.UUID.generate()))
      end
    end
  end
  ...
  describe "sign in" do
    test "given valid credentials", %{conn: conn} do
      ...
      conn = post(conn, Routes.user_path(conn, :signin), %{email: @create_attrs.email, password: @create_attrs.password})
    ...
    test "given invalid credentials or missing parameters (email and/or password)", %{conn: conn} do
      ...
      conn = post(conn, Routes.user_path(conn, :signin), %{email: @create_attrs.email, password: "some wrong email"})
